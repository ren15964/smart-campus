<template>
  <div class="teacher-course-list-container">
      ...
      <el-table :data="courseList" v-loading="loading" border style="width: 100%">
        ...
        <el-table-column label="操作" width="150">
          <template #default="scope">
            ...
            <!-- <el-button type="warning" link @click="handleEditCourse(scope.row)">
              编辑
            </el-button> -->
          </template>
        </el-table-column>
...
const fetchCourses = async () => {
  ...
  try {
    ...
    ElMessage.error('获取教师课程失败，请稍后再试')
  } finally {
    loading.value = false
  }
}
...
const handleViewStudents = (row) => {
  router.push({ name: 'CourseStudents', params: { scheduleId: row.scheduleId } })
}
